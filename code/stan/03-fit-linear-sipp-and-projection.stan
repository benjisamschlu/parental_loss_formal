...
parameters {
  ...
  array[4] vector[cols(ll_X)] m_a_ll_beta;
  // standard deviations
  array[4] real<lower=0> p_a_ll_sigma;
  array[2] real<lower=0> m_a_ll_sigma;
...
model {
  ...
  for (i in 1:2) {
    m_a_ll_beta[i] ~ normal(0, 1);
    m_a_ll_sigma[i] ~ exponential(1);
  }
  for (r in 1:N_RACE) {
    ...
    int X_ind = (r - 1) * N_AGE_C;
    m_a_mom[r] ~ lognormal(
      ll_X[(X_ind + 1):(X_ind + N_AGE_C)] * m_a_ll_beta[1], m_a_ll_sigma[1]);
    m_a_dad[r] ~ lognormal(
      ll_X[(X_ind + 1):(X_ind + N_AGE_C)] * m_a_ll_beta[2], m_a_ll_sigma[2]);
  }
}
