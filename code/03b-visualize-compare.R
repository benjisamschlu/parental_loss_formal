...
plot_line_comp <- function(df, my_theme, 
                           linewidth = .2, 
                           linetype = "dotted", 
                           shape = 20, 
                           size = 1,
...
## Load data -------------------------------------------------------------------
us_mlt_life <- read_csv(here("data", "multistate-lt-life.csv")) |>
    mutate(
...
se_us_mlt_life <- read_csv(here("data", "se-multistate-lt-life.csv")) |>
    mutate(
...
us_mlt_snap <- read_csv(here("data", "multistate-lt-snap.csv")) |>
    mutate(
        quantity = str_remove(name, "_.+"),
        state_to = str_extract(name, "(?<=to_).+"),
        state_from = str_extract(name, "(?<=^[a-zA-Z]{2}_)[a-z]+_[a-z]+"),
    ...
    select(race, x, quantity, state_in, state_from, state_to, value)
se_us_mlt_snap <- read_csv(here("data", "se-multistate-lt-snap.csv")) |>
    mutate(
        name = str_remove(name, "^se_"),
        quantity = str_remove(name, "_.+"),
        state_to = str_extract(name, "(?<=to_).+"),
...
#### SIPP parent status proportions with 95% CIs -------------------------------
plot_pr_comparisons_with_ci <- function(df, race_f, label) {
    df <- df |>
        filter(.data$race == race_f)
    plt_none <- df |> 
        filter(.data$state_in == "lost_none") |>
        plot_line_comp(my_theme_no_legend, se = TRUE) +
        scale_color_brewer(name = "", palette = "Dark2") +
        scale_y_continuous(oob = scales::squish,
                           breaks = c(0, .5, 1),
                           labels = scales::percent) +
...
# plot
g_pr_comp_all_with_ci <- plot_pr_comparisons_with_ci(
    df_pr_comp, "all", 
    grid.text(expression("Proportions by parent mortality states,"~{}[italic(n)]~italic(p)[italic(x)]~"- All races")))
g_pr_comp_hispanic_with_ci <- plot_pr_comparisons_with_ci(
    df_pr_comp, "hispanic", 
    grid.text(expression("Proportions by parent mortality states,"~{}[italic(n)]~italic(p)[italic(x)]~"- Hispanic")))
g_pr_comp_black_with_ci <- plot_pr_comparisons_with_ci(
    df_pr_comp, "non-hispanic black", 
    grid.text(expression("Proportions by parent mortality states,"~{}[italic(n)]~italic(p)[italic(x)]~"- Non-Hispanic black")))
g_pr_comp_white_with_ci <- plot_pr_comparisons_with_ci(
    df_pr_comp, "non-hispanic white", 
    grid.text(expression("Proportions by parent mortality states,"~{}[italic(n)]~italic(p)[italic(x)]~"- Non-Hispanic white")))
g_pr_comp_asian_with_ci <- plot_pr_comparisons_with_ci(
    df_pr_comp, "non-hispanic asian", 
    grid.text(expression("Proportions by parent mortality states,"~{}[italic(n)]~italic(p)[italic(x)]~"- Non-Hispanic Asian")))
# save
png(here("plots", "compare", "pr_all_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_pr_comp_all_with_ci)
dev.off()
png(here("plots", "compare", "pr_hispanic_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_pr_comp_hispanic_with_ci)
dev.off()
png(here("plots", "compare", "pr_black_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_pr_comp_black_with_ci)
dev.off()
png(here("plots", "compare", "pr_white_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_pr_comp_white_with_ci)
dev.off()
png(here("plots", "compare", "pr_asian_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_pr_comp_asian_with_ci)
dev.off()
...
#### SIPP parent mortality rates with 95% CIs ----------------------------------
plot_mx_comparisons_with_ci <- function(df, race_f, label) {
    df <- df |>
        filter(.data$race == race_f)
    plt_none_to_mom <- df |> 
        filter(.data$state_from == "lost_none", state_to == "lost_mom") |>
...
# plot
g_mx_comp_all_with_ci <- plot_mx_comparisons_with_ci(
    df_mx_comp, "all", 
    grid.text(expression("Parent mortality rates,"~{}[italic(n)]~italic(m)[italic(x)]~"- All races")))
g_mx_comp_hispanic_with_ci <- plot_mx_comparisons_with_ci(
    df_mx_comp, "hispanic", 
    grid.text(expression("Parent mortality rates,"~{}[italic(n)]~italic(m)[italic(x)]~"- Hispanic")))
g_mx_comp_black_with_ci <- plot_mx_comparisons_with_ci(
    df_mx_comp, "non-hispanic black",
    grid.text(expression("Parent mortality rates,"~{}[italic(n)]~italic(m)[italic(x)]~"- Non-Hispanic black")))
g_mx_comp_white_with_ci <- plot_mx_comparisons_with_ci(
    df_mx_comp, "non-hispanic white", 
    grid.text(expression("Parent mortality rates,"~{}[italic(n)]~italic(m)[italic(x)]~"- Non-Hispanic white")))
g_mx_comp_asian_with_ci <- plot_mx_comparisons_with_ci(
    df_mx_comp, "non-hispanic asian", 
    grid.text(expression("Parent mortality rates,"~{}[italic(n)]~italic(m)[italic(x)]~"- Non-Hispanic Asian")))
# save
png(here("plots", "compare", "mx_all_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_mx_comp_all_with_ci)
dev.off()
png(here("plots", "compare", "mx_hispanic_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_mx_comp_hispanic_with_ci)
dev.off()
png(here("plots", "compare", "mx_black_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_mx_comp_black_with_ci)
dev.off()
png(here("plots", "compare", "mx_white_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_mx_comp_white_with_ci)
dev.off()
png(here("plots", "compare", "mx_asian_with_ci.png"),
    width = 8, height = 6, units = "in", res = 800)
grid.draw(g_mx_comp_asian_with_ci)
dev.off()
...
legend_parents <- plot_ex_in_state_area(df_ex_life, colours, my_theme) |> 
    get_legend()
plot_ex_comparisons_area <- function(df, colours, race_f, label) {
    ...
    plt_snapshot <- df |>
        ...
        plot_ex_in_state_area(colours, my_theme_no_legend) +
        labs(x = "Age", y = "(years)", caption = "Using snapshot as of 2021")
