...
## Load data -------------------------------------------------------------------
sipp2021 <- read_csv(here("data_private", "sipp_2021-life.csv")) |> # large file
    filter(
        sex %in% SEXES,
        race %in% RACES
    ) |>
...
## Check cell sizes ------------------------------------------------------------
n_by_s_r <- count_samples(sipp2021, by = by_s_r)
n_by_r <- count_samples(sipp2021, by = by_r) |>
    mutate(sex = "all")
n_by_s <- count_samples(sipp2021, by = by_s) |>
    mutate(race = "all")
n_all <- count_samples(sipp2021, by = by_all) |>
    mutate(sex = "all", race = "all")
...
## Save data -------------------------------------------------------------------
write.csv(df_pr, here("data", "sipp_pr-life.csv"), row.names = FALSE)
write.csv(df_mx, here("data", "sipp_mx-life.csv"), row.names = FALSE)
